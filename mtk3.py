...
LABELS = {
    ...
    "ar": {
        ...
        "mtk_confirm_action_msg": "عملية MTK هذه ({action_name}) قد تكون خطيرة وقد تغير حالة الجهاز أو بياناته. تأكد من اختيار الملفات الصحيحة. تابع بحذر.",
        ...
        "mtk_client_not_found_msg": "لم يتم العثور على برنامج mtkclient. يرجى التأكد من تثبيته وإضافته إلى مسار النظام. عمليات MTK لن تكون متاحة.",
        ...
        "cancel_operation_warning_message": "إيقاف العملية بشكل مفاجئ قد يترك الجهاز في حالة غير مستقرة أو يسبب مشاكل. هل أنت متأكد أنك تريد محاولة الإلغاء؟",
...
def load_image(image_path, size=None):
    ...
    try:
        ...
        full_path = os.path.join(base_dir, "assets", image_path) # Assuming assets subfolder
        if not os.path.exists(full_path): # Fallback to current dir if not in assets
...
class ModernButton(tk.Button):
    def __init__(self, master, text, command, theme, width=28, height=1, icon_path=None, icon_size=(16,16), state=tk.NORMAL, relief="raised", borderwidth=2, padx=8, pady=4, **kwargs): # Increased width, relief, bd, padx, pady for "inflated" look
...
class AppController:
    def __init__(self):
        ...
        if app_images.get('app_icon'):
            ...
            try:
                 self.root.iconbitmap(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "assets", "logo.ico"))
            except Exception:
...
class UltimateDeviceTool(tk.Frame):
    ...
    def _check_mtkclient(self):
        ...
        if mtk_exe:
            ...
            home_dir = str(Path.home())
            local_bin_mtk = os.path.join(home_dir, ".local", "bin", "mtk")
            ...
            log_to_file_debug_globally("mtkclient not found in PATH.", "WARNING")
            return None
    ...
    def get_mtk_executable_path(self):
        ...
        if self.app_controller.mtk_client_path:
            ...
            self._setup_mtkclient_path()
            self.app_controller.mtk_client_path = self._check_mtkclient()
            if not self.app_controller.mtk_client_path and self.winfo_exists():
                 messagebox.showwarning(
                    ...
                    self.labels.get("mtk_client_not_found_msg", "mtkclient not found..."),
                    parent=self.master
                )
            return self.app_controller.mtk_client_path
                ...
                if not operation_name.startswith("Get Property"):
                    if return_code == 0:
                        ...
                        if stdout.strip() and not any(kw in stdout.lower() for kw in ["success", "already", "performed", "daemon started successfully"]):
                            # Log mtkclient stdout more extensively as it's often informative
                            ...
                            for line_idx, line_content in enumerate(stdout.strip().splitlines()):
                                if line_idx < 15: # Limit lines to prevent log flooding
                                    ...
                                    break
                        if stderr.strip(): # Also log stderr for mtkclient even on success, as it might contain warnings
    ...
    def fetch_and_log_device_info(self, operation_label_key_on_success, callback_after_info_and_op=None, next_operation_command=None, next_operation_name=""):
        ...
        def _after_next_operation_completed(result):
            ...
            # Progress bar and cancel button handled by main _handle_command_result
...
class SamsungTab(ttk.Frame):
    ...
    def _start_package_disable_sequence(self, packages_to_disable, operation_name_key, success_log_key, failure_log_key):
        ...
        if self.master_app.log_panel and self.master_app.log_panel.winfo_exists():
            # Clear log done by fetch_and_log_device_info or execute_command_async if not part of sequence
            # self.master_app.log_panel.clear_log() # Removed clear_log here
            ...
            self.master_app._update_cancel_button_state(enable=True) # Enable already done if part of sequence
    ...
    def _execute_next_package_disable_step(self):
        if self.current_package_disable_step < self.num_package_disable_steps:
            ...
            if self.master_app.log_panel and self.master_app.log_panel.winfo_exists():
                ...
                self.master_app._update_cancel_button_state(enable=False) # Final disable
    ...
    def _package_disable_step_callback(self, result):
        ...
        if not is_last_step:
            ...
            self._execute_next_package_disable_step() # Call to finalize and log completion
    ...
    def action_remove_mdm_adb(self):
        ...
        self.master_app.fetch_and_log_device_info(
            operation_label_key_on_success="log_mdm_remove_ok", # Not strictly needed if next_op present
            ...
            next_operation_name=self.labels.get("btn_remove_mdm") # This indicates a primary operation
        )
        ...
        def _start_frp_sequence_after_info(info_result):
            if info_result.get("return_code") != 0:
                if self.master_app.log_panel:
                    ...
                    # Progress bar and cancel button are handled by fetch_and_log_device_info's callback system
                return
    ...
    def _frp_step_callback(self, result):
        ...
        if not is_last_step:
            ...
            self._execute_next_frp_step() # Call to finalize
...
class MTKTab(ttk.Frame):
    ...
    def _execute_mtk_action(self, action_name_key, mtk_base_cmd,
                            ...
                            extra_cmd_parts_func=None, # func(input_file, cfg_file) -> list
                            callback_func=None,
                            confirm=True):
        ...
        if extra_cmd_parts_func:
            try:
                extra_parts = extra_cmd_parts_func(input_file, cfg_file, output_dir) # Pass output_dir too
                if extra_parts is None: # Indicates an issue within the function
                    return
                command_parts.extend(extra_parts)
            except TypeError: # Handle if function doesn't expect all args
                 try:
        ...
        if not final_callback:
            def _default_cb(result):
                if self.master_app.log_panel:
                    ...
                    # Failure is handled by main _handle_command_result
    ...
    def action_mtk_auto_boot_repair_dump(self):
        ...
        def callback(result):
            if result.get("return_code") == 0:
                output_dir = result.get("command", [])[-1] # Assuming -o is last
                if not os.path.isdir(output_dir):
                     for i, item in enumerate(result.get("command", [])):
                         if item == "-o" and i + 1 < len(result.get("command",[])):
                             output_dir = result.get("command",[])[i+1]
                             break
                
                if self.master_app.log_panel:
                    self.master_app.log_panel.log(f"Boot Repair Dump: Successfully completed. Files in: {output_dir}", "success")
                
                cfg_file_path = os.path.join(output_dir, "mtk_boot_repair_dump.cfg")
                try:
                    with open(cfg_file_path, 'w', encoding='utf-8') as f:
                        f.write("# Auto Boot Repair Dump Configuration\n")
                        for part_name in boot_partitions:
                            # mtkclient typically saves as {partition}.bin
                            part_bin_name = f"{part_name}.bin"
                            if os.path.exists(os.path.join(output_dir, part_bin_name)):
                                f.write(f"{part_name}={part_bin_name}\n") # Relative path within dump
                    if self.master_app.log_panel:
                        self.master_app.log_panel.log(f"Generated boot repair CFG: {cfg_file_path}", "info")
                except Exception as e:
                    if self.master_app.log_panel:
                        self.master_app.log_panel.log(f"Error generating CFG for boot repair: {e}", "error")
    ...
    def action_mtk_list_partitions(self):
        def callback(result):
            if result.get("return_code") == 0 and self.master_app.log_panel:
                self.master_app.log_panel.log("Partitions list retrieved:", "success")
                stdout = result.get("stdout", "")
                # mtkclient gpt output format: "idx. partition_name: start_addr - end_addr (size)"
                # Or sometimes just "name (size)"
                found_partitions = []
                for line in stdout.splitlines():
                    line = line.strip()
                    # Regex to capture partition name, trying a few patterns
                    match = re.match(r"^\s*\d*\.?\s*([a-zA-Z0-9_-]+)\s*:", line) # e.g., "0. preloader:" or "preloader:"
                    if not match:
                         match = re.match(r"^\s*([a-zA-Z0-9_-]+)\s*\(", line) # e.g., "preloader (size)"
                    
                    if match:
                        part_name = match.group(1)
                        if part_name not in ["GPT", "idx", "Name", "partition_name", "sgpt"]: # Filter out headers/noise
                            found_partitions.append(part_name)
                            self.master_app.log_panel.log(f"  - {part_name}", "info", indent=1)
                if not found_partitions and "GPT header not found" in stdout:
                     self.master_app.log_panel.log("GPT header not found. Cannot list partitions.", "warning")
                elif not found_partitions:
                     self.master_app.log_panel.log("No partitions parsed from output. Check full log.", "warning")

        self._execute_mtk_action("btn_mtk_list_partitions", ["gpt"], callback_func=callback, confirm=False) # List is safe

    def action_mtk_backup_selected_partitions(self):
        op_name = self.labels.get("btn_mtk_backup_selected_partitions")
        partitions = self._get_partitions_from_user(op_name)
        if not partitions: return
        def extra_parts(inp, cfg, out_dir): return partitions
    ...
    def action_mtk_restore_selected_partitions(self):
        # mtkclient uses -cfg for writing partitions from files
        def extra_parts(main_input_file_ignored, cfg_file, out_dir):
            if not cfg_file:
                ...
                return None # Abort
            return ["-cfg", cfg_file] # Main argument is the CFG file for "w" here

        self._execute_mtk_action("btn_mtk_restore_selected_partitions", ["w"],
                                 requires_cfg_file_title_key="mtk_select_cfg_file", # Make it clear it's for partitions
                                 extra_cmd_parts_func=extra_parts)
    ...
    def action_mtk_safe_format(self):
        # This is risky as "safe" partitions vary. mtkclient doesn't have a direct "safe format".
        # We can try formatting common non-userdata partitions.
        # User should be heavily warned.
        safe_partitions_to_format = ["cache", "metadata"] # Example, could be expanded but with care
        if not messagebox.askokcancel(
                self.labels.get("mtk_confirm_action_title"),
    ...
    def action_mtk_erase_frp_write_file(self):
        temp_cfg_file_path_holder = [None] # Use a list to pass by reference

        def extra_parts(frp_file, cfg_file_ignored, out_dir): # cfg_file is ignored, we make our own
            if not frp_file: return None

            try:
                temp_cfg_file = tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.cfg', encoding='utf-8')
                temp_cfg_file.write(f"frp={frp_file}\n")
                temp_cfg_file.close()
                temp_cfg_file_path_holder[0] = temp_cfg_file.name
                return ["-cfg", temp_cfg_file.name]
            except Exception as e:
                if self.master_app.log_panel:
        ...
        def callback_frp_write(result):
            if temp_cfg_file_path_holder[0] and os.path.exists(temp_cfg_file_path_holder[0]):
                try:
                    ...
                    if self.master_app.log_panel: self.master_app.log_panel.log(f"Error deleting temporary FRP CFG: {e}", "warning", indent=1)
            # Default success/fail logging will occur from _handle_command_result

        self._execute_mtk_action("btn_mtk_erase_frp_write_file", ["w"],
                                 requires_input_file_title_key="mtk_select_frp_file",
                                 extra_cmd_parts_func=extra_parts,
    ...
    def action_mtk_generate_ewc(self):
        # This is a multi-step process: 1. Extract keys, 2. (If success) Generate EWC file content.
        ...
        # Step 1: Ask for output directory for keys and EWC
        ...
        if not command_parts_keys: return
        ...
        def _keys_extracted_callback(key_result):
            ...
            try:
                ...
                for item in os.listdir(keys_dir):
                    if item.endswith(".key") or "key" in item.lower(): # Simple check
                ...
                with open(ewc_file_path, 'w', encoding='utf-8') as f_ewc:
                    ...
                    if found_key_files_content:
                        for key_name, key_value in found_key_files_content:
                            f_ewc.write(f"    <key name=\"{key_name.replace('.key','')}\"><![CDATA[{key_value.strip()}]]></key>\n")
                    else:
                        f_ewc.write("    <!-- No specific key files found or parsed -->\n")
                    f_ewc.write("  </extracted_keys>\n")
                    # Could add dump file paths if a dump was made as part of this
                ...
                if self.master_app.log_panel:
                    self.master_app.log_panel.log(f"Error generating EWC file content: {e_ewc}", "error")

        self.master_app.execute_command_async(command_parts_keys, operation_name=f"{op_display_name} (Key Extraction Step)", callback_on_finish=_keys_extracted_callback)


    def action_mtk_build_forensic_project(self):
        op_display_name = self.labels.get("btn_mtk_build_forensic_project")
        # This is a complex sequence. For simplicity, we'll guide the user or perform steps sequentially.
        # 1. Ask for a main project directory.
        # 2. Inside, create subdirs: keys, dumps (security, full_userarea_optional), metadata, ewc.
        # 3. Perform: extract_keys, list_partitions, backup_security_partitions, [optional: read_full_dump], generate_ewc.
        ...
        if not project_base_dir:
            if self.master_app.log_panel: self.master_app.log_panel.log(f"{op_display_name}: Cancelled, no project directory.", "info")
            return
        ...
        # ewc_file will be in project_dir directly
        ...
        try:
            ...
            if self.master_app.log_panel: self.master_app.log_panel.log(f"Error creating project directories: {e}", "error")
            return
        ...
        self.mtk_action_sequences['forensic_project'] = {
            ...
            'current_step': 0,
            'steps': [
                {'name': 'Extract Keys', 'func': self._fp_step_extract_keys},
                {'name': 'List Partitions', 'func': self._fp_step_list_partitions},
                {'name': 'Backup Security Partitions', 'func': self._fp_step_backup_security},
                {'name': 'Generate EWC', 'func': self._fp_step_generate_ewc_for_project},
                {'name': 'Finalize Project', 'func': self._fp_step_finalize}
            ]
        }
    ...
    def _run_next_forensic_project_step(self):
        ...
        if not sequence or sequence['current_step'] >= len(sequence['steps']):
            if self.master_app.log_panel and sequence: # sequence might be None if cancelled early
                 self.master_app.log_panel.log("Forensic project build sequence finished or aborted.", "info")
            return

        step_info = sequence['steps'][sequence['current_step']]
        if self.master_app.log_panel:
             self.master_app.log_panel.log(f"Forensic Project - Step {sequence['current_step'] + 1}: {step_info['name']}", "info", indent=1)
        step_info['func']() # Execute the current step's function

    def _fp_step_callback_factory(self, success_msg):
        def _callback(result):
            ...
            if result.get("return_code") == 0:
                if self.master_app.log_panel:
                    self.master_app.log_panel.log(success_msg, "success", indent=2)
                sequence['current_step'] += 1
            else:
                if self.master_app.log_panel:
                    self.master_app.log_panel.log(f"Step failed. Error: {result.get('stderr', result.get('error', 'Unknown'))}. Aborting project build.", "error", indent=2)
                self.mtk_action_sequences.pop('forensic_project', None) # Stop sequence
            self._run_next_forensic_project_step()
        return _callback

    def _fp_step_extract_keys(self):
        sequence = self.mtk_action_sequences['forensic_project']
        cmd_parts = self._get_common_mtk_cmd_parts(["keys"])
        if not cmd_parts: self._fp_abort(); return
        cmd_parts.extend(["-o", sequence['keys_dir']])
        self.master_app.execute_command_async(cmd_parts, operation_name="FP: Extract Keys",
                                             callback_on_finish=self._fp_step_callback_factory("Keys extracted."),
                                             is_part_of_sequence=True) # Mark as part of sequence for logging

    def _fp_step_list_partitions(self):
        sequence = self.mtk_action_sequences['forensic_project']
        cmd_parts = self._get_common_mtk_cmd_parts(["gpt"])
        if not cmd_parts: self._fp_abort(); return

        def _gpt_callback(result):
            cb_factory = self._fp_step_callback_factory("Partitions listed.")
            if result.get("return_code") == 0:
                try:
                    with open(os.path.join(sequence['metadata_dir'], "partitions_list.txt"), 'w', encoding='utf-8') as f:
                        f.write(result.get("stdout", "No partition data from mtkclient gpt."))
                    if self.master_app.log_panel: self.master_app.log_panel.log("Partition list saved to metadata.", "info", indent=3)
                except Exception as e:
                     if self.master_app.log_panel: self.master_app.log_panel.log(f"Error saving partition list: {e}", "warning", indent=3)
            cb_factory(result) # Call the main step progression callback

        self.master_app.execute_command_async(cmd_parts, operation_name="FP: List Partitions",
                                             callback_on_finish=_gpt_callback,
                                             is_part_of_sequence=True)

    def _fp_step_backup_security(self):
        sequence = self.mtk_action_sequences['forensic_project']
        security_partitions = ["proinfo", "nvram", "nvdata", "nvcfg", "protect1", "protect2", "seccfg", "secro", "metadata", "oemkeystore", "keystore", "frp", "otp"]
        cmd_parts = self._get_common_mtk_cmd_parts(["r"] + security_partitions)
        if not cmd_parts: self._fp_abort(); return
        cmd_parts.extend(["-o", sequence['dumps_dir']])
        self.master_app.execute_command_async(cmd_parts, operation_name="FP: Backup Security",
                                             callback_on_finish=self._fp_step_callback_factory("Security partitions backed up."),
                                             is_part_of_sequence=True)

    def _fp_step_generate_ewc_for_project(self):
        sequence = self.mtk_action_sequences['forensic_project']
        # This step doesn't run a new mtk command, but processes files from previous steps
        ewc_file_path = os.path.join(sequence['project_dir'], f"mtk_forensic_project_{datetime.now().strftime('%Y%m%d_%H%M%S')}.ewc")
        try:
            found_key_files_content = []
            if os.path.exists(sequence['keys_dir']):
                for item in os.listdir(sequence['keys_dir']):
                    if item.endswith(".key") or "key" in item.lower():
                        item_path = os.path.join(sequence['keys_dir'], item)
                        if os.path.isfile(item_path):
                            with open(item_path, 'r', encoding='utf-8', errors='ignore') as kf:
                                found_key_files_content.append((item, kf.read()))
            
            with open(ewc_file_path, 'w', encoding='utf-8') as f_ewc:
                f_ewc.write("<!-- EWC Content for Oxygen Forensic -->\n") # Simplified
                f_ewc.write(f"<ewc_project tool_version=\"{self.labels.get('title')}\" created=\"{datetime.now().isoformat()}\">\n")
                f_ewc.write("  <source_files>\n")
                if os.path.exists(sequence['dumps_dir']):
                    for item in os.listdir(sequence['dumps_dir']):
                        f_ewc.write(f"    <file type=\"partition_dump\" path=\"dumps/{item}\"/>\n")
                f_ewc.write("  </source_files>\n")
                f_ewc.write("  <extracted_keys path=\"keys/\">\n") # Point to keys subdir
                if found_key_files_content:
                    for key_name, key_value in found_key_files_content:
                         f_ewc.write(f"    <key name=\"{key_name.replace('.key','')}\"><![CDATA[{key_value.strip()}]]></key>\n")
                f_ewc.write("  </extracted_keys>\n")
                f_ewc.write("</ewc_project>\n")
            if self.master_app.log_panel: self.master_app.log_panel.log(f"EWC file structure generated: {ewc_file_path}", "success", indent=2)
            sequence['current_step'] += 1
        except Exception as e:
            if self.master_app.log_panel: self.master_app.log_panel.log(f"Error generating EWC for project: {e}", "error", indent=2)
            self.mtk_action_sequences.pop('forensic_project', None)
        self._run_next_forensic_project_step()


    def _fp_step_finalize(self):
        sequence = self.mtk_action_sequences.get('forensic_project')
        if not sequence: return

        readme_path = os.path.join(sequence['project_dir'], "PROJECT_INFO.txt")
        try:
            with open(readme_path, 'w', encoding='utf-8') as f:
                f.write(f"# MTK Forensic Project Information\n")
                f.write(f"# Created: {datetime.now().isoformat()}\n")
                f.write(f"# Project Directory: {sequence['project_dir']}\n\n")
                f.write(f"DA File: {self.da_file_var.get() or 'N/A'}\n")
                f.write(f"Auth File: {self.auth_file_var.get() or 'N/A'}\n")
                f.write(f"Preloader File: {self.preloader_file_var.get() or 'N/A'}\n\n")
                f.write(f"Structure:\n")
                f.write(f"  - keys/      (Extracted device keys)\n")
                f.write(f"  - dumps/     (Partition dumps, e.g., security partitions)\n")
                f.write(f"  - metadata/  (Device info, partition list)\n")
                f.write(f"  - *.ewc      (Oxygen Forensic compatible file, if generated)\n")
            if self.master_app.log_panel: self.master_app.log_panel.log(f"Project README created: {readme_path}", "success", indent=2)
        except Exception as e:
            if self.master_app.log_panel: self.master_app.log_panel.log(f"Error creating project README: {e}", "warning", indent=2)
